...
function handleTouchStart(evento) {
...
function handleTouchMove(evento) {
...
window.addEventListener('load', () => {
    ...
    fetch('./laptop_base.txt')
        ...
        if (tempObj.read(data)) {
            ...
            baseObj = tempObj;
            if (movilObj)
                movilObj.rho = sharedRho; // Sync si tapa cargó primero
            const nameEl = document.getElementById('file-name-base');
            if (nameEl)
                nameEl.innerText = 'laptop_base.txt';
            repaintAll();
        }
    })
    ...
    fetch('./laptop_tapa.txt')
        ...
        if (tempObj.read(data)) {
            ...
            const nameEl = document.getElementById('file-name-movil');
            if (nameEl)
                nameEl.innerText = 'laptop_tapa.txt';
            repaintAll();
        }
    })
    ...
    (_a = document.getElementById('color-pink')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
        if (baseObj) {
            ...
            baseObj.baseColorG = 182;
            baseObj.baseColorB = 193;
        }
        if (movilObj) {
            movilObj.baseColorR = 255;
            movilObj.baseColorG = 182;
            movilObj.baseColorB = 193;
        }
        repaintAll();
    });
    (_b = document.getElementById('color-blue')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', () => {
        if (baseObj) {
            baseObj.baseColorR = 173;
            baseObj.baseColorG = 216;
            baseObj.baseColorB = 230;
        }
        if (movilObj) {
            movilObj.baseColorR = 173;
            movilObj.baseColorG = 216;
            movilObj.baseColorB = 230;
        }
        repaintAll();
    });
    (_c = document.getElementById('color-green')) === null || _c === void 0 ? void 0 : _c.addEventListener('click', () => {
        if (baseObj) {
            baseObj.baseColorR = 152;
            baseObj.baseColorG = 251;
            baseObj.baseColorB = 152;
        }
        if (movilObj) {
            movilObj.baseColorR = 152;
            movilObj.baseColorG = 251;
            movilObj.baseColorB = 152;
        }
        repaintAll();
    });
    ...
    function startRotation(type) {
        ...
        rotInterval = setInterval(() => {
            if (type === 'left') {
                if (baseObj)
                    baseObj.globalRotY -= 0.04;
                if (movilObj)
                    movilObj.globalRotY -= 0.04;
            }
            else if (type === 'right') {
                if (baseObj)
                    baseObj.globalRotY += 0.04;
